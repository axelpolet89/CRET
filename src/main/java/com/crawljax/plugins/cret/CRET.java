...
import com.crawljax.plugins.cret.transformation.*;
...
public class CRET implements OnNewStatePlugin, PostCrawlingPlugin
{
	...
	/**
	 * Enable debug mode, storing additional logs during the crawl and transformation process
	 */
	...
	/**
	 * Function called on detection of new DOM state by Crawljax
	 * Parse CSS from DOM tree and use the CSS selectors to match elements in that tree
	 */
	@Override
	...
	/**
	 * Process discovered CSS files after crawling is complete
	 * Execute defined transformations
	 * Optionally print CSS parse errors
	 * Generate transformed SASS and CSS code
	 * Optionally verify behavior similarity of transformed CSS code
	 * Optionally print statistics on transformations
	 */
	@Override
	...
	/**
	 * Extract external and embedded CSS 'files' from DOM state and parse the CSS into MCssRules
		...
		try
		{
			...
			for (String relPath : CSSDOMHelper.extractCssFileNames(dom))
			{
				...
				if (!_newMcssFiles.containsKey(cssUrl))
				{
					...
					if(_enableVerification || _enableStatistics)
					{
	...
	/**
	 * Parse given string of CSS code
	 */
	...
	/**
	 * Print all parser errors resulted in parsing CSS files with the CssParser
	 */
	...
	/**
	 * Generate File objects to which transformed SASS and CSS code will be written
	 * @return true if all File objects are correctly generated based on original file names
	 */
	...
	/**
	 * Generates CSS code from a given set of MCssFiles
	 * Optionally, it will also generate SASS code including variables and mixins
	 * @return true if no errors occurred during code generation
	 */
	...
	/**
	 * Parse CSS files from code generated by CRET
	 * @return set of parsed CSS files
	 */
	...
	/**
	 * Gather statistics for each transformation and write as XML to file
	 */
	...
	/**
	 * Compute amounts of original and optimzed/transformed CSS selectors -and declarations
	 */
